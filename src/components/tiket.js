...
const HospitalQueueSystem = () => {
  ...
  const confirmSelection = (confirmed) => {
    ...
    if (confirmed && pendingPoli) {
      // Tambah antrian di context (langsung update queueData)
      setQueueData(prev => ({
        ...prev,
        [pendingPoli]: {
          ...prev[pendingPoli],
          total: prev[pendingPoli].total + 1,
          waiting: prev[pendingPoli].waiting + 1
        }
      }));
  ...
  // getQueueNumber tidak perlu update context lagi, hanya handle loading dan show result
